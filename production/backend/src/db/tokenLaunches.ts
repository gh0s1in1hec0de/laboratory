...
export async function updateLaunchCreatorBalance(address: RawAddressString, newCreatorBalance: bigint, client?: Client): Promise<void> {
    const res = await (client || globalClient)`
        UPDATE token_launches SET creator_balance = ${newCreatorBalance} WHERE address = ${address}
    `;
    assert(res.length === 1, `exactly 1 column must be returned, got: ${res}`);
}