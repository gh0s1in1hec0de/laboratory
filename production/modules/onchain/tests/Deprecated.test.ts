...
import { TokenLaunchDeprecated } from "../wrappers/TokenLaunchDeprecated";
import { UserVaultDeprecated } from "../wrappers/UserVaultDeprecated";
import {
    UTIL_JET_SEND_MODE_SIZE, UtilJettonsEnrollmentMode, TokensLaunchOps,
    BASECHAIN, CoreOps, LaunchConfigDeprecated, UserVaultOps, validateValueMock,
...
import { CoreDeprecated } from "../wrappers/CoreDeprecated";
...
describe.skip("V1", () => {
    ...
    let core: SandboxContract<CoreDeprecated>;

    let tokenLaunchCode = new Cell();
    let sampleTokenLaunch: SandboxContract<TokenLaunchDeprecated>;

    let userVaultCode = new Cell();
    let userVault: SandboxContract<UserVaultDeprecated>;
    ...
    // Custom values
    //
    let launchConfig: LaunchConfigDeprecated;
    ...
    beforeAll(async () => {
        ...
        core = blockchain.openContract(
            CoreDeprecated.createFromState(
                {
                    chief: chief.address,
        ...
        sampleTokenLaunch = blockchain.openContract(
            TokenLaunchDeprecated.createFromState({
    ...
    describe("core and launch correct deployment", () => {
        ...
        test("token creation fees measurements", async () => {
            ...
            const { bodyCell, tokenLaunchStateInit } = CoreDeprecated.tokenCreationMessage(
                creator.address, chief.address, utilityJettonMaster.address,
                sampleLaunchParams, code, launchConfig
            );
            const loadedTokenLaunchStateInit = TokenLaunchDeprecated.buildState(
                {
                    creator: creator.address,
                    chief: chief.address,
                    launchParams: sampleLaunchParams,
    ...
    describe("token launch operations", () => {
        ...
        test("loaded user vault state specs", async () => {
            const loadedUserVaultState = UserVaultDeprecated.buildState({
                owner: randomAddress(),
        ...
        test("wl purchase works correctly", async () => {
            ...
            const consumerVault = blockchain.openContract(
                UserVaultDeprecated.createFromState({
