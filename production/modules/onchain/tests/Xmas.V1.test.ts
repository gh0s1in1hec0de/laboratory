...
import {
    ...
    PERCENTAGE_DENOMINATOR, getCreatorAmountOut, UserVaultOps, GlobalVersions,
...
describe("Xmas V1 Launch", () => {
    ...
    let derivedJettonMaster: SandboxContract<XmasJettonMaster>;

    let jettonWalletCode = new Cell();
    let tokenLaunchDerivedJettonWallet: SandboxContract<XmasJettonWallet>;
    ...
    beforeAll(async () => {
        [
            ...
            compile("UserVaultV1"),
            compile("XmasJettonMaster"),
            compile("XmasJettonWallet")
        ]);
    ...
    test("configuring & deployment", async () => {
        const launchInitResult = await sampleTokenLaunch.sendInit(
            { via: creator.getSender(), value: toNano("0.5"), queryId: 0n },
            tokenLaunchDerivedJettonWallet.address,
            derivedJettonMaster.address
        );
        expect(launchInitResult.transactions).toHaveTransaction({
